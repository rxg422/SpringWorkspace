...
public class FileDeleteTask {
	...
	/*
		파일삭제 스케쥴러
			목표 : DB에는 존재하지 않으나, WEB-Server상에만 존재하는 쓸모없는 파일 삭제
			1. DB에서 board_img에 등록된 모든 첨부파일 경로 조회
			2. 모든 게시판 유형(board_type) 조회, 각각의 게시판 디렉터리 경로 탐색
			3. 해당 디렉터리에서 서버에 존재하는 이미지 파일 목록 수집
			4. 각 파일 DB 등록여부 확인
			5. DB에 존재하지 않은 파일이면 삭제 처리
			6. 유저활동량이 적은 매달 1일 4시에 실행되도록 스케쥴링
	*/
...
//	@Scheduled(cron = "1/1 * * * * *")
	public void deleteFile() {
		...
		// 2. 게시판 유형 조회
		List<BoardType> typeList = service.selectBoardType();
