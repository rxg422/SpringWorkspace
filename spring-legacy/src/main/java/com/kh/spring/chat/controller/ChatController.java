...
public class ChatController {
	...
	public String openChatRoom(@ModelAttribute ChatRoom room, Authentication auth, RedirectAttributes ra) {
	...
	public String joinChatRoom(@PathVariable("chatRoomNo") int chatRoomNo, Model model, Authentication auth, ChatRoomJoin join) {
		/*
			업무로직
			1. 채팅방 번호를 통해 치팅방 메세지 내용 조회
			2. 창며자 수 증가(chatRoomJoin 테이블에 데이터 등록)
			3. 채팅방 메세지 model에 추가 후 forward
		*/
		...
		model.addAttribute("chatRoomNo", chatRoomNo); // session scope에 보관
