...
public class BoardServiceImpl implements BoardService {
	...
	/*
	 * @Transactional - 선언적 트랜잭션 관리 어노테이션 - Exception.class와 하위 예외가 발생하면 rollback
	 * 처리한다. - rollbackFor 미지정시 RuntimeException 에러가 발생한 경우만 rollback;
	 */
	@Override
	@Transactional(rollbackFor = { Exception.class })
	public int insertBoard(Board b, List<BoardImg> imgList) {
		/*
		 * 서비스 로직 0. 게시글 데이터 전처리(개행문자 처리 및 xss 핸들링) 1. board 테이블에 데이터 insert 2. 첨부파일 존재시
		 * 첨부테이블에 insert 3. 오류 발생시 rollback
		 */
...
//				}
	...
	public int updateBoard(Board board, String deleteList, List<BoardImg> imgList) {
		...
		if (deleteList != null && !deleteList.equals("")) {
			...
			if(result == 0) {
				throw new RuntimeException("첨부파일 삭제 에러");
			}
		}
